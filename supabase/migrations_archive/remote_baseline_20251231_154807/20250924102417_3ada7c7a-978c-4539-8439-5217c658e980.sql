-- PHASE 1: Clear existing data and restructure tables (Final version)
...
-- Drop existing emoji_profiles table
DROP TABLE IF EXISTS emoji_profiles CASCADE;

-- Drop existing function that references emoji_profiles
...
-- Drop existing RLS policies on fanmarks that depend on user_id
...
-- Create new simplified RLS policies for fanmarks table
...
-- Create RLS policies for config tables (users can manage configs for their licensed fanmarks)
