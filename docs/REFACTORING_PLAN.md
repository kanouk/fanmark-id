...
2. **`register-fanmark`(880行)**: バリデーション / Tier判定 / ライセンス作成 / プロフィール作成 / 通知 を同様にモジュール分割。
   **⚠️ 絵文字→UUID変換の二重実装に注意**: 本来の仕様は「フロントの `emojiConversion.ts` が絵文字文字列を正規化・分割してUUID配列に変換し、以降はUUID配列(`emoji_ids` / `normalized_emoji_ids`)で扱う」だが、この関数には `emoji_ids` 未指定時のサーバー側フォールバック `convertEmojiSequenceToIds`(index.ts:147-179)が存在する。これはフロントと**異なるアルゴリズム**(grapheme分割+ライブの `emoji_master` テーブル照会。フロントはカタログキーの最長一致分割+ビルド時スナップショット)であり、複合絵文字で結果が食い違い得る。**分割リファクタリング時にこのフォールバックを勝手に削除・統一しないこと**(挙動変更になる)。扱いは Phase 8 の残課題として Issue 化する: 推奨は「フロントは常に `emoji_ids` を送っている(FanmarkAcquisition.tsx:218-222)ため、フォールバックを廃止して `emoji_ids` 必須にする」だが、これは仕様判断を要する。
...
3. **残課題の棚卸し**: 本計画でスコープ外としたもの(E2Eテスト拡充、i18nライブラリへの移行是非、`migrations` スナップショット運用の自動化、**絵文字→UUID変換のサーバー側フォールバック廃止の是非**(§6.2-2参照)など)を Issue 化する。
