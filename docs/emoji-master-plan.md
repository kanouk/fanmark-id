...
2025-03 以降の修正で、ファンマ入力は以下の辞書ベース処理を通過する。

1. **サニタイズ**: 入力文字列から空白・`FE0E` を除去。<br />
   `canonicalizeEmojiString()` が入口で必ず適用される。
2. **辞書照合**: `emojiCatalog` の `lookupEntries` を先頭文字単位で探索し、最大 10 文字のスライディングウィンドウで最長一致を抽出。<br />
   一致した部分は `convertEmojiIdsToSequence()` で fully-qualified 表記に復元する。
3. **フォールバック**: 最長一致が見つからない場合は 1 文字ずつ消費して再試行するため、未知のコードポイントがあっても処理が停止しない。<br />
...
- `🙂‍↔️🙂‍↔️🙂‍` → canonical 化で `🙂‍↔️🙂‍↔️🙂‍↔️` に揃い、3 セグメントとして扱われる。
- 旗（例: `🏳️‍🌈`）、家族（例: `👨‍👩‍👧‍👦`）、職業（例: `👩‍⚕️`）など ZWJ を含む複合 emoji は、辞書にある限り自動で fully-qualified へ復元される。
- 辞書に存在しないシーケンスは、`convertEmojiSequenceToIdPair()` がエラーを返し、「この絵文字はサポートされていません」メッセージを表示する。

### 運用メモ

- 辞書を更新した際は、`scripts/test-emoji-conversion.ts` で複合絵文字のラウンドトリップテストを実行し、canonical 化 → ID 変換 → 再構成が成功することを確認する。
- トースト文言の翻訳キーは `common.pasteCompleted`・`common.inputCompleted` を使用。翻訳ファイル (`ja.json` / `en.json`) も同時更新する。
- 開発ログの `segmentInput marker` や canonical count は仕様確認用。不要になったら `NODE_ENV` 条件ごと削除して構わない。
