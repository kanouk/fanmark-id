...
Checkpoint: 2026-09-26. The migration is **not complete**. PR #41 is open and
...
The extension-price admin API code is deployed but its backend selector remains unset. Do not enable or deploy its frontend selector by itself: checkout still reads the extension price and Stripe ID from Supabase, so the admin writer and checkout reader need a paired staging cutover.
...
`1413b726-0930-45f4-b779-67865fffa24d` at
...
re-enabled the shared every-minute Cron. Current Worker version
`1413b726-0930-45f4-b779-67865fffa24d` is active at 100%; the deployed
...
| --- | --- | --- |
| Credential transform target profile | Current worktree | Exact local DDL is bound to the source catalog, lifecycle/generation schema, and descriptor. The special six-column credential writer atomically applies the target row, artifact, coverage, and checkpoint for enabled credentials on active licenses, with typed readback. Current 40-table catalog rehearsal passes 3 synthetic rows, all 40 checkpoints, ACK-unknown resume, whole-target readback, and tampered-coverage rejection; `deployable` and `fullMigrationReconciled` remain false because 18 schema gates remain. Disabled/inactive durable deferred coverage and all real Auth/business/Storage imports remain open and are excluded from the current stage. See `docs/migration/credential-import-integration.md`. |
...
| PostgreSQL event sequence and encrypted backup | Current worktree | Snapshot format 4 captures and verifies the reviewed `fanmark_events.id` sequence definition, exact decimal `lastValue`, and `isCalled`; local D1 import reads back the seeded next ID for called and unused cases. The Git-external backup is one AES-256-GCM ciphertext with source names/counts hidden inside, and restore authenticates, verifies, then publishes only a valid snapshot. Migration-data suite passes 116/116, including 16 D1 importer cases. Synthetic only: no live sequence/user rows were read or migrated. A final source snapshot still requires the event writer to be frozen; operational destination/key custody/retention and persisted-artifact restore remain open. See `docs/migration/snapshot-export-design.md` and `docs/migration/d1-import.md`. |
...
| Application Worker Auth and emoji admin route | Existing staging Worker + local-only follow-up | `/api/auth/*` reaches Better Auth through dedicated `AUTH_DB`; admin routes require role and current-session MFA assurance. The synthetic TOTP/admin canary passed on deployed version `1413b726-0930-45f4-b779-67865fffa24d`; it exercised sign-in, enrollment, session rotation, and gated admin reads, then removed all synthetic Auth rows. Current deployed anonymous probes return `/api/auth/ok` 200 and `/api/admin/session` 401. The local worktree now contains conditional Resend verification/reset and four existing-account OAuth providers; signup and social signup remain disabled. This follow-up has not been redeployed, and the staging selectors/secrets are unset, so deployed signup/email/OAuth/reset remain closed. Broader admin authorization and CPU plan fit remain open. See `docs/migration/auth-feasibility.md` and `live-observations.md`. |
...
| Notification admin and logs | Master editor and payload-minimized read routes deployed to workers.dev staging | Deployment `938f880d-f3db-46d5-9634-60612e4e2814` includes the MFA-gated D1 rule/template editor, event log, and delivery log. A synthetic admin completed TOTP/MFA; the canary read 10/40 masters and both logs. Anonymous requests returned 401, notification payloads were omitted, and delivery user IDs were truncated to eight characters. Notification rows were unchanged. Manual event POST accepts only the three UI event types and passes the local synthetic D1 test; it has not been exercised remotely, so no staging event was inserted. See `docs/migration/notifications-api.md`. |
...
| Other non-user reference masters | Current worktree + APAC staging D1/Worker | Migrations `0004` and `0006` are applied; release generation 2 has 4 tiers, 4 languages, 5 reserved patterns, and 16 extension prices. All four public routes return HTTP 200/no-store and omit Stripe IDs. The private HMAC route is deployed. The versioned MFA-protected editor API code is deployed but its Worker/frontend selectors remain unset until checkout moves with it; the public master release remains unchanged. The Edge Function secret/checkout selectors remain unset, so checkout stays on Supabase. See `docs/migration/reference-master-data.md` and `live-observations.md`. |
...
| Current app staging deployment | APAC `fanmark-app-staging` Worker + Static Assets | Current version `1413b726-0930-45f4-b779-67865fffa24d` at 100%; split D1 and both R2 bindings remain. The staging SPA selects analytics writer/owner reads and notification-admin Worker APIs. Synthetic TOTP/MFA canary passed on this version, including master and event/delivery log reads. The reference-master editor selector is unset pending paired checkout migration. Public reference masters return 200/no-store; the admin pricing route is gated off. The every-minute Cron remains for notification/Stripe dispatch; a separate `0 0 * * *` lifecycle trigger is configured with `LICENSE_EXPIRY_BACKEND` unset. No real user data, production routing, or domain/DNS changed. Signup/email/OAuth, remaining admin routes/event sources, real user/Auth/object import, production routing, and domain/DNS remain open. |
...
4. The #35 encrypted snapshot exporter and local synthetic D1 restore path now
   ...
   names/counts and 64 KiB-rounded size metadata. After the #37 synthetic
   application acceptance, complete the remaining backup destination, key
   custody, retention/deletion, and persisted-artifact restore checks. Run the
   real snapshot only in #38's final user-data operation: source freeze,
...
- R2 is enabled and two empty APAC Standard staging buckets are bound to the
  staging Worker. The frontend still points to Supabase, and no source Storage
  inventory or object transfer has occurred. Standard R2 includes 10 GB-month
  storage, 1 million Class A, and 10 million Class B operations each month;
  usage beyond those allowances is billed. The tiny synthetic smoke was within
  the included allowance; the Cloudflare invoice/dashboard was not inspected.
...
- Real-data compatibility, encrypted backup/restore, source freeze/drain,
  Auth/OAuth/MFA transfer, production Storage provisioning/object transfer, and
  single-writer cutover remain open.
