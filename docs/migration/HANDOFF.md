...
Staging now enables the public D1 extension-price reader and the Better Auth/MFA-gated D1 reference-master editor together. The active 16-row price release is unchanged; the live price route returned 200/no-store without Stripe IDs and anonymous admin access returned 401. No authenticated edit has been sent. The Stripe extension-checkout route remains deliberately disabled and returns 404 because server/webhook/dispatch selectors and Stripe secrets are absent; no payment was attempted. Production/default builds remain on Supabase.
...
migration. The auth email-template seed and admin plan/suspension rollouts below
...
`2758095a-99fd-4aaf-a1bf-afc441276f05` at
...
| --- | --- | --- |
| Credential transform target profile | Current worktree | Exact local DDL is bound to the source catalog, lifecycle/generation schema, and descriptor. The special six-column credential writer atomically applies the target row, artifact, coverage, and checkpoint for enabled and disabled credentials on active licenses, with typed readback. Current 40-table catalog rehearsal passes 3 synthetic rows, all 40 checkpoints, ACK-unknown resume, whole-target readback, and tampered-coverage rejection; a separate full-import synthetic test confirms disabled passwords are hashed, `is_enabled=0` is retained, and `disabled` coverage reads back. Inactive-license rows remain deferred; `deployable` and `fullMigrationReconciled` remain false with 18 schema gates unresolved. All real Auth/business/Storage imports remain out of the current stage. See `docs/migration/credential-import-integration.md`. |
...
| Other non-user reference masters | Current worktree + APAC staging D1/Worker | Migrations `0004` and `0006` are applied; release generation 2 has 4 tiers, 4 languages, 5 reserved patterns, and 16 extension prices. All four public routes return HTTP 200/no-store and omit Stripe IDs. The private HMAC route is deployed. The staging build selects the public D1 price reader and the MFA-protected D1 editor together. A live synthetic TOTP admin read matched all four tiers and 16 prices to active Master D1; anonymous access was 401 and both reads left the active pointer unchanged. No authenticated edit was sent. Stripe extension checkout remains disabled and returns 404 until server/webhook/dispatch selectors and secrets are configured. Production/default pricing remains on Supabase. See `docs/migration/reference-master-data.md` and `live-observations.md`. |
...
| Current app staging deployment | APAC `fanmark-app-staging` Worker + Static Assets | Current version `2758095a-99fd-4aaf-a1bf-afc441276f05` at 100%; split D1 and both R2 bindings remain. Business migrations through `0015` and Auth migration `0008_auth_user_suspension.sql` are applied; all eight user-owned Auth tables, including status audit, read back empty after the latest TOTP canary. The 16 localized auth email master rows remain readback-verified against their pinned content/seed digests. MFA-gated user list/detail, plan, and suspension/restoration operations use split D1. The live canary verified suspension, current-session revocation, restoration, and audit, then removed its synthetic account and status rows. Signup and email delivery remain disabled because delivery is not configured; OAuth providers remain unset. The every-minute Cron remains for notification/Stripe dispatch; the separate daily lifecycle trigger is configured with its execution selector unset. No real user data, production routing, or domain/DNS changed. Password-reset and immediate-expiry mutations, remaining app/API inventory, Stripe sandbox/integrated acceptance, real user/Auth/object import, production routing, and domain/DNS remain open. |
