...
| Current app staging deployment | APAC `fanmark-app-staging` Worker + Static Assets | Current version `bc5ad53e-5f08-492b-81fb-8046c9be9600` at 100%; split D1 and both R2 bindings remain. The staging SPA selects analytics writer/owner reads and notification-admin Worker APIs. Existing synthetic TOTP/MFA canary remains prior-version evidence; no Auth user rows were created by the latest deployment probes. The reference-master editor selector is unset pending paired checkout migration. Public reference masters return 200/no-store; the admin pricing route is gated off. The every-minute Cron remains for notification/Stripe dispatch; a separate `0 0 * * *` lifecycle trigger is configured with `LICENSE_EXPIRY_BACKEND` unset. No real user data, production routing, or domain/DNS changed. Signup/email/OAuth, remaining admin routes/event sources, real user/Auth/object import, production routing, and domain/DNS remain open. |
