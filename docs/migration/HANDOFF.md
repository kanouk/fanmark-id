...
| Application Worker Auth and emoji admin route | Current worktree + staging Worker | `/api/auth/*` reaches Better Auth through dedicated `AUTH_DB`; admin routes require role and current-session MFA assurance. The synthetic TOTP/admin canary passed on current version `1413b726-0930-45f4-b779-67865fffa24d`; it exercised sign-in, enrollment, session rotation, and gated admin reads, then removed all synthetic Auth rows. Current anonymous probes return `/api/auth/ok` 200 and `/api/admin/session` 401. Signup, email delivery, OAuth, and password reset remain closed/hidden. Broader admin authorization and CPU plan fit remain open. See `docs/migration/auth-feasibility.md` and `live-observations.md`. |
