...
2. Continue #36's emoji-master path: the verified UUID-bearing release was
   ...
   release path with identity guards. Reconcile user-held references, then
   stage the master and deploy the app to an isolated Cloudflare staging
   environment before #37, after verifying the Wrangler account and plan/CPU
   fit. Use synthetic users only; do not attach the production domain or import
   real user data. The staging release can be exercised in #37; production
   activation remains part of #38. No remote D1 write, deployment, or public
