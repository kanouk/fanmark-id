...
Staging enables the public D1 extension-price reader and Better Auth/MFA-gated D1 reference-master editor together. A synthetic TOTP administrator edited Tier C from null to one day and restored it to null through the deployed API. Anonymous writes returned 401, a stale-version write returned 409 without changing the active release, and exact readback matched all four reference masters after restoration. The active pointer is now generation 6 after two reversible canaries; the append-only activation history retains both edit/restore pairs. Synthetic Auth/profile rows were removed and read back at zero. The Stripe extension-checkout route remains deliberately disabled and returns 404 because server/webhook/dispatch selectors and Stripe secrets are absent; no payment was attempted. Production/default builds remain on Supabase.
...
rehearsal; reference-master release generation 6 remains active.
...
| Other non-user reference masters | Current worktree + APAC staging D1/Worker | Migrations `0004` and `0006` are applied; active generation 6 has 4 tiers, 4 languages, 5 reserved patterns, and 16 extension prices. All four public routes return HTTP 200/no-store and omit Stripe IDs. The private HMAC route is deployed. A synthetic TOTP admin edited Tier C null→1→null through the deployed MFA API; anonymous access returned 401, stale CAS returned 409, all four masters matched after restore, and two activation pairs remain in immutable history. Auth/profile business canary rows returned to zero. Stripe extension checkout remains disabled and returns 404 until server/webhook/dispatch selectors and secrets are configured. Production/default pricing remains on Supabase. See `docs/migration/reference-master-data.md` and `live-observations.md`. |
