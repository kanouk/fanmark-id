...
| Subscription reconciliation | Local D1 handling for created/updated/deleted events retrieves current Stripe Subscription/Customer state under a customer-generation fence, requires exact customer mapping or verified Customer metadata, uses private test/live Price IDs, upserts current subscription projections, derives active-only plan entitlement, clears payment-failure fields on an active update, and commits ledger/fence/receipt/dispatch atomically. A last-subscription deletion sets Free and returns newest excess licenses with audit and owner/favorite outbox events in the same transaction; an active transfer blocks the entire batch. Migrations 0008–0012 are applied to empty staging D1. Worker version `a67b6abe-0080-4784-bcf2-87efed59f83a` is active at 100% and contains invoice/subscription reconciliation plus the Free-to-paid plan Checkout route; Stripe selectors/secrets remain unset, so billing routes stay closed. | Current-state stale-event handling, active-only entitlement, test/live mapping, deterministic multi-active plan order, deletion with and without another active subscription, newest-excess order, configured/default Free limit, transfer blocking, atomic rollback, dispatcher routing, missing mapping, and owner conflict. The plan Checkout route now has owner-bound Customer/Session command rows and is frontend-connected in staging, but live Stripe sandbox acceptance remains required. Still needed: paid-plan change commands and integrated staging acceptance. The Portal start route is deployed but disabled until its selector and Stripe keys are configured. |
...
Existing paid-plan changes remain to be implemented.
