...
| admin-toggle-user-status | E: supabase/functions/admin-toggle-user-status/index.ts | Worker admin | Medium: preserve account state transition audit |
...
| Stripe checkout, customer portal, and webhooks | Retain Stripe; Worker owns authenticated initiation and idempotent webhook projection into user_subscriptions | PRODUCT:73-84,453-482; D1 webhook/reconciliation staging runtime; Worker customer portal route; plan checkout/change commands remain |
