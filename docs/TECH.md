...
- Cloudflare Worker側には`/api/stripe/webhook`の署名検証・D1 receipt/dispatch保存経路、拡張Checkout作成API、D1 scheduled dispatcherと拡張決済効果を追加した。staging business D1にはmigrations `0006`〜`0010`を適用済み。`0009`でStripe subscription IDの全ユーザー横断unique index、`0010`で作成/更新イベントの照合台帳を追加した。`STRIPE_WEBHOOK_BACKEND=d1`、`STRIPE_DISPATCH_BACKEND=d1`、`STRIPE_EXTENSION_CHECKOUT_BACKEND=d1`、署名secretとCronは未設定。scheduled invoice照合は`STRIPE_SECRET_KEY_TEST`と`STRIPE_SECRET_KEY_LIVE`をモード別に要求する（拡張Checkout作成APIが使う`STRIPE_SECRET_KEY`とは別）。Workerのローカル実装はcreated/updated subscriptionを現在のStripe状態・customer fence・private test/live Price IDで照合し、activeのみプランを反映してpayment failure項目をクリアする。subscription.deletedのfree移行とライセンス返却は未実装で、今回のreconciliation codeは未配備。稼働中Workerは旧version `68a2e0bf-3236-444c-9c7a-a46294037855`。Stripe selectors/secretsは未設定で、Stripe Dashboardのendpoint切替も未実施。
...
2026-09-25の`fanmark-app-staging`ではBetter Authのlogin/sessionを合成アカウントで確認し、Auth D1を分離した。Cloudflare Secret Storeにはstaging専用secretを登録し、リポジトリやビルド成果物には含めない。signup、メール検証/送信/リセット、OAuthは未接続で閉じている。実ユーザーのAuth/業務データ移行やproduction認証切替を意味しない。

管理画面の招待コードCRUDには、`INVITATION_ADMIN_BACKEND=d1`と`VITE_INVITATION_ADMIN_BACKEND=worker`で選ぶMFA保護API/clientを追加した。招待コードは未移行のため両selectorはstagingで無効。コード検証・消費とsignupはAuth D1/business D1をまたぐ一貫性およびメール確認フローが未実装のため引き続き閉じている。設計と試験は `docs/migration/invitation-admin-api.md` を参照。
