...
待機リスト管理は`workers/api/src/waitlist-admin-d1-api.ts`のD1 APIを使う。`WAITLIST_ADMIN_BACKEND=d1`と`VITE_WAITLIST_ADMIN_BACKEND=worker`を選んだstaging経路では、Better Authの同一セッションMFAとD1 `user_settings.plan_type='admin'`の両方を要求する。`GET /api/admin/waitlist`は最大100行のハッシュ済みメールとpayloadを含めない監査概要を返し、`GET /api/admin/waitlist/:id/email`は監査insert成功後に限り個別アドレスを返す。Worker障害時にSupabaseへフォールバックせず、通常buildの既定selectorはSupabase。行移行は未実施で、登録経路、保持・削除方針も未移行・未決定。契約は `docs/migration/waitlist-admin-api.md`。
