...
統合プレビューの検索エリアは、選択スロットの直下へ直接入力・貼り付け・クリア・Undoをまとめ、その下にアドレスプレビュー/空き状況/お気に入り/絵文字コピーを配置する。クリアはUndo可能。装飾バッジとサンプルカードは置かず、選択した絵文字を大きく見せる。コピーする対象は絵文字列であることを明示する。
