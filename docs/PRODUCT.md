...
- 検索/取得: Grace 中のファンマは「取得できません」表示（取得済み扱いにしない）。
...
| 抽選申込中 | ✅ 可 | 延長が優先され、pending エントリーはキャンセル |
...
## Web版絵文字パレットの試作（本番仕様への採用前）

Issue #26 の操作確認用サンプルを `prototypes/emoji-palette/` に置く。検索語を保持した連続追加、入力済み全体の共有、指定位置への挿入、1個の置き換え、削除、並べ替え、Undo、端末内履歴を試せる。最大5個。空スロットを飛ばした場合は末尾へ詰めて追加し、保存する絵文字列に穴を作らない。追加は即時反映し、置き換え選択時はパレットを閉じる。パレット内の「すべてクリア」は入力中の絵文字を一括削除し、次の追加位置を先頭に戻す。検索語と履歴は保持し、「ひとつ戻す」で削除前の並び・操作位置を復元できる。

日本語・英語の既存名称/キーワードによる文字一致を即時表示し、Jev実APIの「カテゴリ→絵文字」の階層検索結果を先頭に追加する。手作業の連想辞書は使用せず、Jevに渡す候補も文字一致の結果には依存しない。JevのON/OFF、通信状況の表示、失敗時の辞書検索を備える。検索語をTypeSafeへ送ることをパレットに明示し、履歴・入力済み絵文字は送信しない。検索品質と対応言語は試験段階。単独版では空き状況確認・取得・課金・本番の入力欄の置換は行わない。

文章検索では「主題」「場所」「気持ち」「関連する道具」「身体的な反応」「比喩・象徴」をそれぞれ独立してカテゴリから探し、候補に残す。例として「東京の美味しいラーメン」から 🍜・🗼・😋・🤤 を提案する。文章を代表する1個だけに絞らず、ユーザーが表したい要素を選べるようにする。場所は地名の完全一致だけでなく、その街を連想させる景色・文化・食べ物・服装などを候補にする。絵文字によるイメージ提案であり、地理的な識別や現地情報の保証には使わない。未知の地名や、入力から読み取れない場所・気持ちは無理に補わない。比喩・象徴は、文中の抽象的な概念や技術を一般的な視覚表現で示すために使う（例: AIを 🤖 で表す）。具体的な道具を探す経路とは分け、弱い連想は該当なしにできる。

## 箱庭ヒーローの試作（本番採用前）

Issue #27 の候補Aを `prototypes/hero-world/` で試す。実際のトップのヒーローだけを差し替え、花屋・カフェ・歩く人と動物が暮らす小さな惑星を表示する。全体を収めず街に寄り、ドラッグ回転・ズーム・停止・リセットを備える。12キャラクターと8店舗を全球に分散配置し、人と動物はランダムな行き先へ球面上を歩く。頭上の吹き出しと店の看板には公開ファンマを固定で割り当て、不足分は試作用絵文字を補い、クリックでコピーする。時間による入れ替えはしない。キャラクターは装飾で、実所有者の人物像を表すものではない。

下のファンマスクロール、サンプル説明や住人一覧、SMALL PLANET等の世界観を定義する文言、ドラッグ・ズーム等の操作注釈は表示しない。曲がる道・木立・ベンチ・街灯・花鉢で街並みを補い、建物は人との比率を大きくする。静止画は廃止し、3D失敗時は再試行と既存検索を使える構成にする。ヘッダー・検索・使い方・フッター・認証・課金の既存画面を共有する。通常の本番エントリは従来ヒーローのままで、本番導入前に見た目と実機性能を評価する。

### 新パレットと検索エリアの統合プレビュー

ポート4179の試作では、単独パレットの見た目と操作をベースに既存の検索エリアへ接続する。選択と直接入力・貼り付けを同じ絵文字列へ同期し、最大5個と正規化を維持する。空き状況・取得・お気に入り・抽選・アクセス・認証導線は既存処理を使う。連続選択中は旧検索結果を表示せず、最新入力の結果だけを採用する。本番標準の入力欄は変更せず、採用はCloudflare全面移行 #28 の後。統合の確認範囲と未検証項目は `prototypes/hero-world/README.md` に記録する。

統合プレビューは「選択する場所」と「取得へ進む場所」を分ける。PCは左に大きな絵文字スロット/編集操作、右に小さな取得状態と主ボタンを置く。狭い画面では同じ順序で縦に並ぶ。取得可能なら既存の取得/ログイン導線、取得不可なら「組み合わせを変える」でパレットを開く。5個未満は追加、5個なら先頭の置き換えから始める。既存の登録条件・認証・抽選・上限判定は維持する。状態は小さなテキスト、アドレスと絵文字コピーは補助表示。直接入力・貼り付け・クリア・Undoとお気に入り/アクセス/抽選導線を残す。

統合版ではホバー時の途中挿入「＋」を撤去する。追加は空き枠、順序変更は絵文字のドラッグ＆ドロップで行い、移動先を強調する。キーボード/タッチ用の左右ボタンとUndoを維持。検索エリア/下部CTA背景は単色とし、案内エリアの装飾グラデーションも除去する。

統合プレビューのトップ全体は白/生成り/落ち着いたグリーンで統一する。ヒーロー・検索・用途・3ステップ・登録案内・フッターの横幅、見出し、余白、ボタンをそろえ、用途カードの色付き影や見出しの装飾を抑える。3ステップは番号付きの案内、下部登録案内は簡潔な横並びとし、狭い画面では縦に並ぶ。既存文言の翻訳と操作は保持し、他の画面には適用しない。
