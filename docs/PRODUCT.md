...
Issue #27 の候補Aを `prototypes/hero-world/` で試す。ミント・ピンク・温かい白の小さな街に絵文字の看板を置き、人やお店への入口を表現する。看板またはHTMLの選択ボタンから絵文字を選び、アドレス例を表示・コピーできる。看板はサンプルで、取得可能性を示さない。日英表示、動きの停止、静止画切り替えを備える。本番検索・取得・認証・課金・最近取得データには未接続。導入前に見た目の確認と実機の性能評価を行う。
