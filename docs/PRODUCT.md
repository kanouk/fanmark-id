...
Issue #27 の候補Aを `prototypes/hero-world/` で試す。実際のトップのヒーローだけを差し替え、花屋・カフェ・歩く人と動物が暮らす小さな惑星を表示する。全体を収めず街に寄り、ドラッグ回転・ズーム・停止・リセットを備える。18キャラクターと8店舗を全球に分散配置し、人と動物はランダムな行き先へ球面上を歩く。頭上の吹き出しと店の看板には公開ファンマを固定で割り当て、不足分は試作用絵文字を補い、クリックでコピーする。時間による入れ替えはしない。キャラクターは装飾で、実所有者の人物像を表すものではない。
