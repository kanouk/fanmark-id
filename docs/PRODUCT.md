...
下のファンマスクロール、サンプル説明や住人一覧、SMALL PLANET等の世界観を定義する文言は表示しない。静止画は廃止し、3D失敗時は再試行と既存検索を使える構成にする。ヘッダー・検索・使い方・フッター・認証・課金の既存画面を共有する。通常の本番エントリは従来ヒーローのままで、本番導入前に見た目と実機性能を評価する。
