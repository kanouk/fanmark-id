...
日本語・英語の既存名称/キーワードによる文字一致を即時表示し、Jev実APIの「カテゴリ→絵文字」の階層検索結果を先頭に追加する。手作業の連想辞書は使用せず、Jevに渡す候補も文字一致の結果には依存しない。JevのON/OFF、通信状況の表示、失敗時の辞書検索を備える。検索語をTypeSafeへ送ることをパレットに明示し、履歴・入力済み絵文字は送信しない。検索品質と対応言語は試験段階。空き状況確認・取得・課金・本番の入力欄の置換は行わない。
